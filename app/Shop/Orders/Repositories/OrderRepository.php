...
class OrderRepository extends BaseRepository implements OrderRepositoryInterface
{
     ...
     */
    public function sendEmailToCustomer($password = null)
    {
        Mail::to($this->model->customer)
            ->send(new SendOrderToCustomerMailable($this->findOrderById($this->model->id), $password));
    }
    ...
    public function sendEmailNotificationToAdmin()
    {
        ...
        //  $employee2 = $employeeRepo->findEmployeeById(2);
    ...
    {
        return $this->model->Customers($customer_id)->latest()->get();
    }
    ...
    public function getOrderDetail($where, $value) 
    {
        return $this->model->with([
            'orderStatus', 'detail', 'medicalHistory', 'address' => function($query){
                $query->with(['country']);
            }, 'customer'
        ])->where($where, $value)->first();
    }
}
